...
Only `@pourkit/cli` is public; the `pourkit` binary is the installed executable, and `@pourkit/logger` remains private and bundled into the CLI. Publishing is performed by GitHub Actions from the repo root with npm workspaces, `NPM_TOKEN`, `--access public`, and provenance. User-facing changes require a Changeset; internal-only changes use an explicit `no-changeset-needed` label.

## Consequences

- Normal feature PRs target `next`; `main` receives promotion, version, and emergency hotfix PRs only.
