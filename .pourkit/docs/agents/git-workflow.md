...
- **Release PRs** (`next -> master`): use **merge commit** (not squash). This preserves the underlying `feat:` / `fix:` history on `master` for `git bisect` and `semantic-release`.
- **Normal PRs** (topic branch -> `next`): squash or merge commit, any preference.

## After merge

- **Hotfix to master**: the Recreate-next workflow automatically brings the fix back to `next`.

## Release workflow

After a stable release PR from `next` to `master` is merged, the following happens automatically:

1. **Release workflow** runs on `master` via `.github/workflows/release.yml`
   - `semantic-release` analyzes commits, creates the stable version tag, updates `CHANGELOG.md`, and publishes the GitHub Release
2. **Recreate-next workflow** runs on completion of the Release workflow via `.github/workflows/recreate-next-after-release.yml`
   - Resets `refs/heads/next` to the released `master` commit
   - This is needed because GitHub auto-deletes `next` after the release PR is merged, and we want `next` based on the latest stable release

The Recreate-next workflow uses the `workflow_run` trigger (not `on: release`) to avoid the GITHUB_TOKEN trigger suppression issue: events created by GITHUB_TOKEN do not trigger new workflow runs, but `workflow_run` fires when a referenced workflow completes regardless of what token was used.
