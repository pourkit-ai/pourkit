...
If the target is `master` for a release PR from `next`, use merge commit (`--merge`) instead of squash. Then switch back to `$original_branch`.

If the user chose wait and merge, wait for checks and merge only after they pass. Use Pourkit's Octokit-backed merge command with squash for normal PRs:

```bash
pourkit pr merge <number>
```

Release PRs targeting `master` should use merge commit:
