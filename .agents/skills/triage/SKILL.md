...
- [Issue tracker operations](../../../.pourkit/docs/agents/issue-tracker.md) — how to create, read, comment, label, and close GitHub Issues
...
- `blocked` — issue has unresolved dependencies, not yet runnable

Five **priority** roles (AFK-track only):

- `type:bugfix` — critical bugfix (priority 1)
- `type:infra` — development infrastructure (priority 2)
- `type:feature` — new feature or tracer bullet (priority 3)
- `type:polish` — polish and quick wins (priority 4)
- `type:refactor` — refactor (priority 5)

Every triaged issue should carry exactly one category role and one state role. If state roles conflict, flag it and ask the maintainer before doing anything else.

AFK-track issues must also carry exactly one priority role (`type:*`). If a `ready-for-agent` issue is missing a `type:*` label or has more than one, flag it.

The `blocked` role is optional. Apply it when an issue has unresolved dependencies. Remove it when all dependencies are clear.

A `ready-for-agent` issue with `blocked` means: fully specified but not yet runnable.
A `ready-for-agent` issue without `blocked` means: runnable by Pourkit.

A dependency is considered resolved when the blocking issue is closed.
