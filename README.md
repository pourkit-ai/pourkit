...
- Ongoing work goes into PRs targeting `next`.
- `next` auto-publishes prereleases.
- When a PRD is complete, open a PR from `next` to `master` for the official release.
- PRs must use conventional commit titles and squash merge.
- `semantic-release` runs automatically on pushes to `next` and `master`.
- This repo is on GitHub's free private-repo plan, so branch protection cannot be enforced here; CI checks and team process are the release gate for now.
